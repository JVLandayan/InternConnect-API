...
namespace InternConnect.Service.Main
{
    ...
    public class IsoCodeService : IIsoCodeService
    {
        ...
        public IsoCodeService(IIsoCodeRepository isoCodeRepository, IMapper mapper, InternConnectContext context,
            IAdminRepository adminRepository, IAccountRepository accountRepository)
        {
        ...
        public IList<IsoCodeDto.ReadIsoCode> GetAllByAdminId(int adminId)
        {
            ...
            foreach (var isoCode in isoCodeList) mappedList.Add(_mapper.Map<IsoCodeDto.ReadIsoCode>(isoCode));
        ...
        public IList<IsoCodeDto.ReadIsoCode> GetAllByProgramId(int programId)
        {
            var isoCodeList = _isoCodeRepository.GetAllCodesWithRelatedData()
                .Where(i => i.ProgramId == programId && i.Admin.AuthId == 3);
            var accountList = _accountRepository.GetAll().ToList();
            IList<IsoCodeDto.ReadIsoCode> mappedList = new List<IsoCodeDto.ReadIsoCode>();
            foreach (var isoCode in isoCodeList) mappedList.Add(_mapper.Map<IsoCodeDto.ReadIsoCode>(isoCode));
            foreach (var item in mappedList)
        ...
        public void TransferIsocodeToChair(IList<IsoCodeDto.TransferIsoCode> payload, int programId)
        {
            var isoCodeList = new List<IsoCode>();
        ...
        public void TransferIsocodeToCoordinator(IsoCodeDto.TransferIsoCode payload, int adminId)
        {
            var isoCodeData = _isoCodeRepository.Get(payload.Id);
        ...
        {
            var isoCodeList = new List<IsoCode>();
            var isoCodeListFromDb = _isoCodeRepository.GetAll().Where(i => i.ProgramId == payload.First().ProgramId)
                .ToList();
